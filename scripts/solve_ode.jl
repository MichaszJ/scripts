...
function _rk_matrices(solver_method)
    if solver_method == "rk_45"
        a = @SVector [0, 1/4, 3/8, 12/13, 1, 1/2]
        b = @SMatrix [
            0 0 0 0 0;
            1/4 0 0 0 0;
            3/32 9/32 0 0 0;
        ...
        c = @SVector [16/135, 0, 6656/12825, 28561/56430, -9/50, 2/55]
        cs = @SVector [25/216, 0, 1408/2565, 2197/4104, -1/5, 0]
    
    elseif solver_method == "rk_56"
        a = @SVector [0, 1/6, 4/15, 2/3, 4/5, 1, 0, 1]
        b = @SMatrix [
            0 0 0 0 0 0 0;
            1/6 0 0 0 0 0 0;
        ...
        c = @SVector [7/1408, 0, 1125/2816, 9/32, 125/768, 0, 5/66, 5/66]
        cs = @SVector [31/384, 0, 1125/2816, 9/32, 125/768, 5/66, 0, 0]
    
    elseif solver_method == "rk_78"
        a = @SVector [0, 2/27, 1/9, 1/6, 5/12, 1/2, 5/6, 1/6, 2/3, 1/3, 1, 0, 1]
        b = @SMatrix [
        ...
        c = @SVector [41/840, 0, 0, 0, 0, 34/105, 9/35, 9/35, 9/280, 9/280, 41/840, 0, 0]
        cs = @SVector [0, 0, 0, 0, 0, 34/105, 9/35, 9/35, 9/280, 9/280,0, 41/840, 41/480]
    end
...
function rk_solver(ode_func, y_init, t_init, t_final; solver_method="rk_45", tolerance=100.0, beta=0.8)
    a, b, c, cs = _rk_matrices(solver_method)
